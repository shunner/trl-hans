...
首先，我们定义什么是_表达式（expression）_。表达式可以是一个_原子（atom）_，即一串英文字母（如 foo)，也可以是包在一对小括号中，由空格分隔的，零个或多个表达式组成的_列表（list）_。如下皆为表达式：
...
最后的表达式是个拥有四个元素的列表，而其第三个元素本身又是一个拥有单个元素的列表。

在算术运算中，表达式 1 + 1 取值为 2。有效的 Lisp 表达式也有取值。如果表达式 _e_ 取值为 _v_，我们就说 _e 返回（return） v_。接下来我们将定义都有哪些表达式，以及它们各自的返回值是什么。

对于列表这种表达式，我们称第一个元素为_操作符（operator）_，其余的元素为_实际参数（argument，以下简称实参）_。我们将定义七个基本（相当于预设的公理）操作符：quote、atom、eq、car、cdr、cons 和 cond。
...
反之，一个被引用的列表仅被视为列表。在此例中就是拥有两个元素的列表：
...
引用，可能看上去有点陌生，因为几乎没有其他语言有类似的概念。它和 Lisp 最与众不同的特征之一紧密相连：代码和数据由相同的数据结构构成, 而我们借助 quote 操作符来区分它们。

3. (eq _ x_ _ y_)返回t如果_ x_和_ y_的值是同一个原子或都是空表, 否则返回().
...
4. (car _ x_)期望_ x_的值是一个表并且返回_ x_的第一个元素.
...
5. (cdr _ x_)期望_ x_的值是一个表并且返回_ x_的第一个元素之后的所有元素.
```
> (cdr '(a b c))
(b c)
```

6. (cons _ x_ _ y_)期望_ y_的值是一个表并且返回一个新表,它的第一个元素是_ x_的值, 后
面跟着_ y_的值的各个元素.
...
(b c)
```
7. (cond (\pone\dots\eone) \dots (\pn\dots\en)) 的求值规则如下. _ p_表达式依次求值直到有一个
返回t.  如果能找到这样的_ p_表达式,相应的_ e_表达式的值作为整个cond表达式的
返回值.
