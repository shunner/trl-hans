...
[<a name="footnote1">1</a>]: "Recursive Functions of Symbolic Expressions and Their Computation by Machine, Part1." _Communication of the ACM_ 3:4, April 1960, pp. 184-195.
  ...
  在本文中，我将试着用最简单的术语来解释 John McCarthy 的发现。重点不仅在于领会四十年前某人取得的有趣理论成果，还在于展示编程语言的前进方向。Lisp 的不同寻常之处——也就是它决定性的特质——在于它可以由自己编写而成。为了理解 McCarthy 这句话的含义，我们将追随他的足迹，同时将他的数学标记转换成可运行的 Common Lisp 代码。
...
随着时间的推移，中庸语言，即中庸程序员所使用的语言，始终在向 Lisp 靠拢。因此通过理解 eval 你始终能够很好地理解主流计算模式将会是什么。
...
在 McCarthy 的论文中，假值用 f 来表示，而不是空列表。我用 () 表示假值以使示例代码能在 Common Lisp 中运行。代码并不依赖于假值也恰好是空列表；条件判断返回的结果并没有被加入任何内容。
