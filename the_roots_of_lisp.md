...
既然理解了 eval 的工作原理，让我们回过头来考虑一下这意味着什么。我们拥有了一个简约的计算模型。仅用 quote、atom、eq、car、cdr、cons 和 cond 就定义了函数 eval.，真正实现了我们的语言，从而可以随心所欲地定义更多函数。
...
随着时间的推移，中级语言，即被中间层程序员使用的语言，正一致地向 Lisp 靠近。因此通过理解 eval 你能够很好理解将来的主流计算模式会是什么样的。
