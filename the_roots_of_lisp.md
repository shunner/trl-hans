...
eval. 比我们在前文见过的函数定义都长。我们考察一下每个部分各自的作用。

  该函数接受两个实参：e 是即将被求值的表达式，而 a 是一个列表，以类似函数调用中实参的形式列出对每个原子赋予的值。这种形如 pair. 返回值的列表叫做_环境（environment）_。正是为了构造和查找这种列表，我们才编写了 pair. 和 assoc.。

  eval. 的主干是一个带有四个子句的 cond 表达式。我们根据表达式的类型来决定对其求值的方式。第一个子句处理原子。如果 e 是原子，我们就在环境中寻找它的值：
...
eval. 第二个子句的结尾部分负责处理那些被当作参数传进来的函数的调用。它把原子替换为对应的值（应该是 lambda 或 label 表达式）再对得到的结果表达式求值。于是
